...
import io.github.sgpublic.dormnet.targets.template.DefaultDormnetDevice
...
object CQCAI : UserPwdDeviceTarget() {
    ...
    override fun invoke() {
        ...
        LaunchedEffect(Unit) {
            viewModel.username = Config[CqcaiUsername] ?: ""
            viewModel.password = Config[CqcaiPassword] ?: ""
            Config[CqcaiDevice]?.let {
                viewModel.device = DormnetDevice.valueOf(it)
            }
        }
    }
    ...
    override suspend fun doLogin(params: UserPwdDeviceTargetParamsData): Result<String> {
        Config {
            it[CqcaiUsername] = params.username
            it[CqcaiPassword] = params.password
            it[CqcaiDevice] = params.device.name
        }
        return runCatching {
    ...
    private suspend fun requestLogin(
        params: UserPwdDeviceTargetParamsData,
        parameters: CqcaiNetworkInfo,
        ...
        val response = HttpClient.get("https://auth.cqcai.edu.cn:802/eportal/portal/login") {
            ...
            parameter(
                "user_account",
                ",${params.device.accountPrefix},${params.username}".encodeBase64(),
            )
            parameter("user_password", params.password.encodeBase64())
            parameter("wlan_user_ip", parameters.wlanuserip)
            parameter("wlan_user_ipv6", "")
            parameter("wlan_user_mac", parameters.mac.replace(":", "").lowercase())
            parameter("wlan_ac_ip", parameters.wlanacip)
            parameter("wlan_ac_name", parameters.wlanacname)
            parameter("jsVersion", "4.2.1")
            parameter("terminal_type", params.device.terminalType)
...
private val CqcaiUsername = stringPreferencesKey("cqcai_username")
private val CqcaiPassword = stringPreferencesKey("cqcai_password")
private val CqcaiDevice = stringPreferencesKey("cqcai_device")
